...
parser.add_argument('-shape', '--shape', type=str, help="model input shape, same as used blob",
                    choices=["120x160", "160x240"], required=True)
...
# 创造最终输出的节点
nn_xout = pipeline.createXLinkOut()#第一个
nn_xout.setStreamName("nn")

xout = pipeline.create(dai.node.XLinkOut)#第二个
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        nn_disp_vis = cv2.putText(nn_disp_vis, f"NN FPS {fps:.2f}", (20, 20), font, fontScale, color, thickness,
                                  cv2.LINE_AA)
        stereo_disp_vis = cv2.putText(stereo_disp_vis, f"Stereo Disp", (20, 20), font, fontScale, color, thickness,
                                      cv2.LINE_AA)
